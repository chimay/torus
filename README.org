...
If you want to create another circle, simply launch ~torus-add-circle~
again, and enter another name (or it won’t be created).
...
These functions are bound in a prefix map. The prefix key is <super-t>
by default. I strongly suggest you bind the most used of these
functions to quick shortcuts. See the [[*Key bindings][Key bindings]] below.
...
All default bindings are available after the prefix key <super-t>. You
can see them by pressing <super-t><C-h>, or by installing [[https://github.com/justbur/emacs-which-key][which-key]].

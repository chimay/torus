...
Split until `torus-maximum-horizontal-split' is reached."
  ...
  (let* ((circle (cdr (car torus-torus)))
    ...
    (if (> numsplit (1- torus-maximum-horizontal-split))
        (message "Two many files to split.")
    ...
    (let ((dist-dec-hor)
      ...
      (while (not getout)
        ...
        (cond ((and (<= dist-dec-hor dist-dec-ver)
               ...
               (when (> torus-verbosity 1)
                 (message "Dec hor : int-hor int-ver = %d %d" int-hor  int-ver)))
              ((and (>= dist-dec-hor dist-dec-ver)
                    (>= (* int-hor (1- int-ver)) len-circle))
               (setq int-ver (1- int-ver))
               (when (> torus-verbosity 1)
                 (message "Dec ver : int-hor int-ver = %d %d" int-hor  int-ver)))
              (t (setq getout t)))))
    (if (or (> int-hor torus-maximum-horizontal-split)
            (> int-ver torus-maximum-vertical-split))
        (message "Two many files to split.")

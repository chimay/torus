...
(defun torus--jump-to-window (buffer)
  ...
  (when (and (> (length (torus--windows)) 1)
           (not torus-running-split))
    (let* ((location (torus--root-location))
           (position (cdr location))
           (bufwins (duo-filter (torus--windows)
                                (lambda (elem) (equal buffer (window-buffer elem)))))
           (window (car bufwins)))
      (when (> torus-verbosity 1)
      ...
      (let ((delta-win)
            (delta-window))
        (dolist (win bufwins)
          (setq delta-win (abs (- (window-point win) position)))
          (setq delta-window (abs (- (window-point window) position)))
          (when (> torus-verbosity 1)
              (message "delta-win %s delta-window %s" delta-win delta-window))
          (when (< delta-win delta-window)
            (setq window win)
            (when (> torus-verbosity 1)
              (message "win %s window %s" win window)))))
      (when window
